...
using Bundlingway.Core.Interfaces;
using Bundlingway.Core.Services;
using Bundlingway.Core.UI;
using Bundlingway.Model;
using Bundlingway.Core.Utilities;
using static Bundlingway.Core.Constants;
using Bundlingway.Utilities.Extensions;
using Serilog;
using Bundlingway.Utilities; // For Bootstrap, CustomProtocolHandler
...
namespace Bundlingway.UI.WinForms
{
    public partial class frmLanding : Form, ILandingView
    {
        ...
        public void InitializeServices(IServiceProvider serviceProvider)
        {
            ...
            _ = Announce(MessageCategory.ApplicationStart.ToString());

            txtDesktopShortcut.Text = ProcessHelper.CheckDesktopShortcutStatus();
            txtBrowserIntegration.Text = CustomProtocolHandler.IsCustomProtocolRegistered(GPosingwayProtocolHandler);

            ProcessHelper.NotificationReceived += ProcessHelper_NotificationReceived;
            _ = ProcessHelper.ListenForNotifications();

            _ = UpdateElements(); // If implemented, otherwise call the method directly

            // Announce ready
            _ = Announce(MessageCategory.Ready.ToString());
        }
        ...
        {
            _ = Announce(((Control)sender).Tag.ToString());
        }
        ...
        private void ProcessHelper_NotificationReceived(object sender, IPCNotification e)
        {
            switch (e.Topic)
            {
                ...
                case "package-installed":
                    _ = Announce(e.Message);
                    break;
                case "duplicated-instances":
                    _ = BringToFrontForm();
                    break;
            }
        }
        ...
        private void btnInstallPackage_Click(object sender, EventArgs e)
        {
            ...
            {
                ofd.Filter = "Archive files (*.zip;*.rar;*.7z)|*.zip;*.rar;*.7z|All files (*.*)|*.*";
                ofd.Title = "Select a Package File";
                ofd.Multiselect = true;
                if (ofd.ShowDialog() == DialogResult.OK)
                {
                    RunSafeAsync(() => _presenter.OnInstallPackageAsync(ofd.FileNames));
                }
            }
        }
        ...
        {
            var repositoryPath = _envService.PackageFolder;
            if (string.IsNullOrEmpty(repositoryPath) || !Directory.Exists(repositoryPath))
            {
                MessageBox.Show("Package Folder not found or path is empty.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            _presenter.OpenPackagesFolder(repositoryPath);
        }

        private void btnGameFolder_Click(object sender, EventArgs e)
        {
            var gamePath = _configService.Configuration?.Game?.InstallationFolder;
            if (string.IsNullOrEmpty(gamePath) || !Directory.Exists(gamePath))
            {
                MessageBox.Show("Game Folder not found or path is empty.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            _presenter.OpenGameFolder(gamePath);
        }
        ...
        internal async Task UpdateElements()
        {
            ...
            txtXivPath?.Invoke((Action)(() => { txtXivPath.Text = txtGamePathText; }));

            if (mustDetect)
            {
                txtReShadeStatus?.Invoke((Action)(() =>
                {
            ...
            {
                btnUpdate?.Invoke((Action)(() => { btnUpdate.Visible = true; }));

                _ = Announce($"A new Bundlingway version ({c.Bundlingway.RemoteVersion}) is out!");
            }
            else
            {
                btnUpdate?.Invoke((Action)(() => { btnUpdate.Visible = false; }));
            }
            ...
            if (reShadeBtnVisible)
            {
                if (c.ReShade.Status != EPackageStatus.NotInstalled)
                {
                    ...
                    _ = Announce("If you want to update ReShade, shut down the game first!");
                }
            }
            ...
            txtGPosingwayStatus?.Invoke((Action)(() =>
            {
        ...
        private async Task OpenPackageFolder()
        {
            ...
            try
            {
                ...
                {
                    await Announce("Package folder not found!");
                }
            }
            catch (Exception ex)
            {
                await Announce($"Could not open folder: {ex.Message}");
            }
        }
        ...
        private async Task RenamePackage()
        {
            ...
            if (!string.IsNullOrWhiteSpace(newName) && newName != currentName)
            {
                try
                {
                    ...
                    // Update UI
                    _ = PopulateGridAsync();
                    await Announce($"Package renamed to \"{newName}\"");
                }
                catch (Exception ex)
                {
                    await Announce($"Could not rename package: {ex.Message}");
                }
            }
        }
        ...
        {
            btnInstallPackage?.Invoke((Action)(() =>
            {
