...
namespace Bundlingway.UI.WinForms
{
    public partial class frmShortcuts : Form
    {
        ...
        private string GetShortcutKeyFromSettings(string textBoxName)
        {
            ...
            if (!string.IsNullOrEmpty(safeTextBoxName) && DefaultShortcuts.TryGetValue(safeTextBoxName, out var shortcutKey4) && shortcutKey4 != null)
            {
                return shortcutKey4;
            }
        ...
        {
            // You can implement a label or status bar update here if needed
            // For now, just show a tooltip or message box for demonstration
            ToolTip toolTip = new ToolTip();
            toolTip.Show(message, this, 2000);
        }
        ...
        private async void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (_mustPropagate)
            {
                Announce("Shortcuts saved! Updating installed presets...");
                _ = _packageService.ScanPackagesAsync();
                Announce("Installed presets updated!");
            }
        ...
        {
            Announce(((Control)sender).Tag?.ToString() ?? string.Empty);
        }
